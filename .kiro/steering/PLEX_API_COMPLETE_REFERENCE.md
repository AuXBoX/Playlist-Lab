...
Each Plex token is associated with a specific user account. When making API requests:

- The token determines which user's context the request runs in
- Playlists, libraries, and preferences are user-specific
- Admin users have elevated permissions for server management
- Managed users (Plex Home) have their own tokens and isolated data

### Accessing Other Users' Data

To access another user's playlists or data:

1. **Admin Approach**: Use the admin's token to query user information
2. **User-Specific Tokens**: Each managed user has their own authentication token
3. **Server-Side Logic**: Applications can implement their own user management and data sharing logic

**Important**: The Plex API doesn't provide a direct "switch user" endpoint. Applications must:
- Store and manage tokens for different users
- Make separate API calls with each user's token to access their data
- Implement their own authorization logic for cross-user operations (like playlist sharing)

### Getting User Information

Check token validity and get user details:
```
...
- `thumb` - User's avatar
- `restricted` - Whether user has restrictions
- `home` - Whether user is part of Plex Home

### Plex Home Users

Plex Home allows multiple user profiles on a single Plex account. Each managed user:
- Has their own authentication token
- Has isolated watch history and preferences
- Can have restricted access to content
- Shares the same Plex Media Server

**Note**: To work with multiple Plex Home users in your application, you need to:
1. Authenticate each user separately (they each get their own token)
2. Store tokens securely for each user
3. Use the appropriate token when making requests on behalf of that user
...
Plex Home allows a single Plex account to manage multiple user profiles (managed users). These users can have their own playlists, watch history, and preferences.
...
Returns list of managed users associated with the Plex Home account. Only the admin (account owner) can access this endpoint.
...
- `admin` - Whether user is admin (1 or 0)
- `restricted` - Whether user has restrictions (1 or 0)
- `guest` - Whether user is a guest (1 or 0)

#### Switch User / Get User Token
```
POST https://plex.tv/api/v2/home/users/{userId}/switch
Headers: X-Plex-Token: {adminToken}
```

Switches to a managed user and returns their authentication token. This allows the admin to access resources as that user.

Response includes:
- `authToken` - Token for the managed user
- User profile information

#### Access Managed User's Playlists

Once you have a managed user's token (from switch user endpoint), you can access their playlists using the standard playlist endpoints:

```
GET /playlists?playlistType=audio
Headers: X-Plex-Token: {managedUserToken}
```

This returns playlists owned by that specific managed user.

#### Important Notes

- Only the admin (Plex Home owner) can list all home users
- Only the admin can switch to other users' contexts
- Each managed user has their own separate playlists, libraries, and preferences
- Managed users cannot access other users' playlists unless explicitly shared
- When sharing playlists between users, you need to copy the playlist to the target user's account
...
### Playlist Generators (Smart Playlists / Optimizers)

Playlist generators are used for smart playlists and optimization queues. They define rules for automatically populating playlists.

#### Get Playlist Generators
```
GET /playlists/{playlistId}/generators
```
Returns all generators associated with a playlist.

Response includes `PlayQueueGenerator` array with:
- `id` - Generator ID
- `playlistID` - Associated playlist ID
- `uri` - Library URI with query rules (filters, sorts)
- `type` - Generator type (-1 for standard)
- `createdAt`, `updatedAt`, `changedAt` - Timestamps

#### Get Single Generator
```
GET /playlists/{playlistId}/items/{generatorId}
```
Returns details for a specific generator (used for optimized versions).

Response includes:
- `Status` - Processing status (itemsCount, state, etc.)
- `MediaSettings` - Video quality, bitrate, resolution settings
- `Policy` - Scope and rules (all/unwatched)
- `Location` - Library URI and section ID
...
```
Modify a playlist generator. Used for optimizer configuration.

Parameters (as query string):
- `Item[type]` - Generator type
- `Item[title]` - Generator title
- `Item[targetTagID]` - Target device tag ID
- `Item[Location][uri]` - Library URI
- `Item[Policy][scope]` - Scope (all/unwatched)

#### Get Generator Items
```
GET /playlists/{playlistId}/items/{generatorId}/items
```
Returns the actual media items produced by a generator.

#### Reprocess Generator
```
PUT /playlists/{playlistId}/items/{generatorId}/{metadataId}/{action}
```
Actions: `reprocess`, `disable`, `enable`

Triggers generator to refresh/reprocess items.

#### Delete Generator
```
DELETE /playlists/{playlistId}/items/{generatorId}
```
Removes a generator from a playlist.

#### Add Generator to Playlist
```
PUT /playlists/{playlistId}/items?uri={libraryUri}
```
For smart playlists, passing a new `uri` parameter replaces the rules for the playlist.
